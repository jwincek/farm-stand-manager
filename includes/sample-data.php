 ...
 *
 * All sample posts are tagged with a '_pkit_sample_data' meta key
...
const SAMPLE_META_KEY = '_pkit_sample_data';

/**
 * Check if sample data is currently loaded.
 */
function is_loaded(): bool {
	return (bool) get_option( 'pkit_sample_data_loaded', false );
}
...
add_action(
	...
	function (): void {
		...
		if ( isset( $_GET['pkit_sample_action'] ) && wp_verify_nonce( $nonce, 'pkit_sample_action' ) ) {
			$action = sanitize_key( wp_unslash( $_GET['pkit_sample_action'] ) );

			if ( $action === 'load' && ! is_loaded() ) {
				seed_all();
				update_option( 'pkit_sample_data_loaded', true );
				wp_safe_redirect( admin_url( 'admin.php?page=producerkit&pkit_sample=loaded' ) );
				exit;
			}

			if ( $action === 'remove' && is_loaded() ) {
				remove_all();
				delete_option( 'pkit_sample_data_loaded' );
				wp_safe_redirect( admin_url( 'admin.php?page=producerkit&pkit_sample=removed' ) );
				exit;
			}
		}
	}
...
function get_dashboard_html(): string {
	...
	$result = isset( $_GET['pkit_sample'] ) ? sanitize_key( wp_unslash( $_GET['pkit_sample'] ) ) : '';
...
function seed_locations(): int {
	...
	update_post_meta( $id, '_pkit_address', '123 Farm Road, Yourtown, ST 00000' );
	update_post_meta( $id, '_pkit_location_type', 'stand' );
	update_post_meta( $id, '_pkit_venmo_handle', 'examplefarm' );
	update_post_meta(
		$id,
	...
	update_post_meta( $id, '_pkit_hours', 'Saturdays 1:00 – 4:00 PM, May – December' );
	...
	if ( ! is_wp_error( $market_id ) ) {
		...
		update_post_meta( $market_id, '_pkit_address', 'Courthouse Square, Jonesborough, TN 37659' );
		update_post_meta( $market_id, '_pkit_location_type', 'market' );
		update_post_meta( $market_id, '_pkit_hours', 'Saturdays 8:00 AM – 12:00 PM, May – October' );
	}
...
function seed_events( int $location_id, array $product_ids ): void {
	...
	if ( ! is_wp_error( $pizza_id ) ) {
		...
		update_post_meta( $pizza_id, '_pkit_start_datetime', $pizza_date . 'T18:00:00' );
		update_post_meta( $pizza_id, '_pkit_end_datetime', $pizza_date . 'T21:00:00' );
	...
	if ( ! is_wp_error( $seed_id ) ) {
		...
		update_post_meta( $seed_id, '_pkit_start_datetime', $seed_date . 'T11:00:00' );
	...
	if ( ! is_wp_error( $tour_id ) ) {
		...
		update_post_meta( $tour_id, '_pkit_start_datetime', $tour_date . 'T10:00:00' );
