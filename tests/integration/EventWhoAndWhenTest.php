...
final class EventWhoAndWhenTest extends WP_UnitTestCase {
	...
	public function test_the_words_follow_the_trade(): void {
		...
		$this->assertSame( 'Support Acts', MetaLabels\label( '_pkit_em_also_appearing' ) );
		$this->assertSame( 'Doors', MetaLabels\label( '_pkit_em_doors_datetime' ) );

		update_option( 'pkit_producer_profile', [ 'farm' ] );
		$this->assertSame( 'Sharing the booth', MetaLabels\label( '_pkit_em_also_appearing' ) );
		$this->assertSame( 'Gates open', MetaLabels\label( '_pkit_em_doors_datetime' ) );
	}

	public function test_a_trade_with_no_override_gets_the_neutral_word(): void {
		update_option( 'pkit_producer_profile', [ 'general' ] );

		$this->assertSame( 'Also appearing', MetaLabels\label( '_pkit_em_also_appearing' ) );
		$this->assertSame( 'Doors open', MetaLabels\label( '_pkit_em_doors_datetime' ) );
	}
	...
	public function test_the_meta_keys_are_the_same_for_everyone(): void {
		...
		$id = $this->event();
		update_option( 'pkit_producer_profile', [ 'musician' ] );
		update_post_meta( $id, '_pkit_em_also_appearing', 'The Hold Steady' );

		update_option( 'pkit_producer_profile', [ 'farm' ] );

		$this->assertSame( 'The Hold Steady', get_post_meta( $id, '_pkit_em_also_appearing', true ) );
	}

	/* ── Doors ──────────────────────────────────────────── */

	public function test_doors_before_the_start_are_shown(): void {
		$id = $this->event( '2026-09-12T19:00:00' );
		update_post_meta( $id, '_pkit_em_doors_datetime', '2026-09-12T18:30:00' );

		$this->assertSame( '2026-09-12T18:30:00', Meta\doors_datetime( $id ) );
	}
	...
	public function test_doors_at_or_after_the_start_are_not_shown(): void {
		...
		update_post_meta( $id, '_pkit_em_doors_datetime', '2026-09-12T21:00:00' );
		$this->assertSame( '', Meta\doors_datetime( $id ) );

		update_post_meta( $id, '_pkit_em_doors_datetime', '2026-09-12T19:00:00' );
		$this->assertSame( '', Meta\doors_datetime( $id ) );
	}
	...
	public function test_what_was_typed_is_still_there(): void {
		...
		$this->assertSame( '2026-09-12T21:00:00', get_post_meta( $id, '_pkit_em_doors_datetime', true ) );
	}
	...
	public function test_a_malformed_doors_value_is_refused(): void {
		$id = $this->event();
		update_post_meta( $id, '_pkit_em_doors_datetime', 'half past six' );

		$this->assertSame( '', get_post_meta( $id, '_pkit_em_doors_datetime', true ) );
	}
	...
	public function test_a_ticket_link_is_kept(): void {
		$id = $this->event();
		update_post_meta( $id, '_pkit_em_ticket_url', 'https://example.test/tickets' );

		$this->assertSame( 'https://example.test/tickets', get_post_meta( $id, '_pkit_em_ticket_url', true ) );
	}
	...
	public function test_a_javascript_url_is_refused(): void {
		...
		$id = $this->event();
		update_post_meta( $id, '_pkit_em_ticket_url', 'javascript:alert(1)' );

		$this->assertSame( '', get_post_meta( $id, '_pkit_em_ticket_url', true ) );
	}
	...
	public function test_age_restriction_is_free_text(): void {
		...
		foreach ( [ '18+', 'All ages', 'Under-12s with an adult' ] as $value ) {
			update_post_meta( $id, '_pkit_em_age_restriction', $value );
			$this->assertSame( $value, get_post_meta( $id, '_pkit_em_age_restriction', true ) );
		}
	}
	...
	public function test_the_rest_response_carries_them(): void {
		...
		update_post_meta( $id, '_pkit_em_also_appearing', 'The Hold Steady' );
		update_post_meta( $id, '_pkit_em_doors_datetime', '2026-09-12T18:30:00' );
		update_post_meta( $id, '_pkit_em_age_restriction', '18+' );
		update_post_meta( $id, '_pkit_em_ticket_url', 'https://example.test/tickets' );
		...
		$this->assertSame( '2026-09-12T18:30:00', $detail['event']['doors'] );
